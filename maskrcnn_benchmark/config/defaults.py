...
_C.ONEFLOW_PYTORCH_COMPARING.SAVE_MODEL_NAME_2_OPTIMIZER_STATE = False
# dump momentum buffer
_C.ONEFLOW_PYTORCH_COMPARING.APPOINTED_LR = -1.0
_C.ONEFLOW_PYTORCH_COMPARING.MANIPULATE_MOMENTUM_AND_WEIGHT_DECAY = False
_C.ONEFLOW_PYTORCH_COMPARING.APPOINTED_WEIGT_DECAY = 0.0
_C.ONEFLOW_PYTORCH_COMPARING.APPOINTED_MOMENTUM = 0.0
_C.ONEFLOW_PYTORCH_COMPARING.SAVE_MODEL_INITED = False


def print_cfg_diff(cfg1, cfg2, path=""):
    for k in cfg1.keys():
        if k not in cfg2:
            raise ValueError

        if type(cfg1[k]) is CN:
            sub_path = None
            if path == "":
                sub_path = k
            else:
            ...
            print_cfg_diff(cfg1[k], cfg2[k], sub_path)
        else:
            if cfg1[k] != cfg2[k]:
                print(path, ":")
                print(" - ", k, " : ", cfg1[k])
